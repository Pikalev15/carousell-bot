...
test("serves core and roadmap API endpoints", async () => {
  ...
  try {
    ...
    const priceTarget = await post(`${base}/api/price-targets`, { query: "rtx 3070", target_price: 300, active: true });
    const priceTargets = await getJson(`${base}/api/price-targets`);
    const targetCommand = await handleTelegramCommand("/target gpu under 250");
    const targetsCommand = await handleTelegramCommand("/targets");
    const systemHealth = await getJson(`${base}/api/system/health`);
    const searchDiagnostics = await getJson(`${base}/api/search/diagnostics`);
    const callbackLabel = await handleTelegramCommand({ type: "callback", action: "bad_deal", listingId: 1, id: "cb-1", chatId: "42" });
    const callbackWorth = await handleTelegramCommand({ type: "callback", action: "worth", listingId: 1, id: "cb-1b", chatId: "42" });
    const callbackExpensive = await handleTelegramCommand({ type: "callback", action: "too_expensive", listingId: 1, id: "cb-1c", chatId: "42" });
    ...
    assert.equal(priceTarget.query, "rtx 3070");
    assert.equal(priceTarget.target_price, 300);
    assert.equal(priceTargets.some((target) => target.query === "rtx 3070"), true);
    assert.match(targetCommand, /Price target saved/);
    assert.match(targetsCommand, /gpu under S\$250|gpu under S\$250/);
    assert.equal(systemHealth.ok, true);
    assert.equal(typeof systemHealth.counts.price_targets, "number");
    assert.equal(Array.isArray(searchDiagnostics.recent_searches), true);
    assert.match(callbackLabel, /(Marked|Trained as) bad deal/);
    assert.match(callbackWorth, /(Marked good|Trained as good deal)/);
    assert.match(callbackExpensive, /(Marked bad deal|Trained as overpriced)/);
