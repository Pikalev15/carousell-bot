(() => {
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", installScraperIdeasLabV2);
  } else {
    installScraperIdeasLabV2();
  }

  function installScraperIdeasLabV2() {
  ...
  async function runAdvancedSearch() {
    ...
    const payload = {
      query,
      mode: $("search-more")?.dataset?.active === "true" ? "more" : "web",
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent("scraper-ideas-search-complete", { detail: result }));
